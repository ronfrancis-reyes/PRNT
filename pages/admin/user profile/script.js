...
if (dom.cancelEditBtn) dom.cancelEditBtn.addEventListener('click', () => exitEdit(false));
...
if (dom.pwConfirm) {
    dom.pwConfirm.addEventListener('input', () => {
        if (dom.pwMatchError) {
            dom.pwMatchError.style.display = (dom.pwConfirm.value && dom.pwNew && dom.pwConfirm.value !== dom.pwNew.value) ? 'flex' : 'none';
        }
    });
}
...
function syncSidebar(avatarUrl = null) {
    ...
    const email    = dom.fieldEmail    ? dom.fieldEmail.value.trim()    : profileData.email;
    const phone    = dom.fieldPhone    ? dom.fieldPhone.value.trim()    : profileData.phone;
    const location = dom.fieldLocation ? dom.fieldLocation.value.trim() : profileData.location;
...
function togglePwPanel(open) {
    ...
    if (dom.pwToggle) dom.pwToggle.setAttribute('aria-expanded', String(state.pwOpen));
...
function enterEdit() {
    ...
    dom.editInputs.forEach(i => { i.disabled = false; i.classList.add('editable'); });
...
function exitEdit(save) {
    if (!save) {
        ...
        if (dom.fieldLocation) dom.fieldLocation.value = state.snapshot.location || '';
    }
    dom.editInputs.forEach(i => { i.disabled = true; i.classList.remove('editable'); });
...
function saveProfile() {
    ...
    if (dom.fieldPhone)    profileData.phone    = dom.fieldPhone    ? dom.fieldPhone.value.trim()    : profileData.phone;
    if (dom.fieldLocation) profileData.location = dom.fieldLocation ? dom.fieldLocation.value.trim() : profileData.location;
...
function handlePasswordSubmit() {
    ...
    if (!cur)          { showToast('Required', 'Enter your current password.', 'warning'); dom.pwCurrent && dom.pwCurrent.focus(); return; }
    if (npw.length < 8){ showToast('Too Short', 'New password needs at least 8 characters.', 'warning'); dom.pwNew && dom.pwNew.focus(); return; }
    if (npw !== conf)  { showToast('Mismatch', 'Passwords do not match.', 'danger'); dom.pwConfirm && dom.pwConfirm.focus(); return; }
