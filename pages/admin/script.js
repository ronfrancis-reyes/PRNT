...
// API SERVICE LAYER (SAFE WRAPPER)
// DO NOT replace existing logic yet
window.ApiService = {
    get(url) { return fetch(url).then(res => res.json()); },
    post(url, data) { return fetch(url, { method: 'POST', body: JSON.stringify(data) }).then(res => res.json()); }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function (e) {
            ...
            if (targetSection === currentSection && targetFolder === currentFolder) return;
            ...
            if (targetFolder === 'user profile') {
                document.querySelector('.sidebar-profile[data-folder="user profile"]')
            ...
            if (pageTitle) {
                pageTitle.textContent =
                    targetFolder === 'user profile' ? 'Admin Profile'
                    : targetTitle || this.querySelector('.sb-label')?.textContent
    ...
    window.navigateTo = function (section) {
        ...
        if (link) link.click();
        else console.warn(`[PRNT] navigateTo: no nav link found for section "${section}"`);
    };
    ...
    // ============================================================
    // SECTION: SIDEBAR TOGGLE (mobile hamburger)
    // ============================================================
    ...
    document.getElementById('menuToggle')
        ?.addEventListener('click', toggleAdminSidebar);
    document.getElementById('sidebarOverlay')
        ?.addEventListener('click', toggleAdminSidebar);

    // Prevent icon child elements from blocking click propagation
    document.querySelectorAll('.sidebar-nav a i')
    ...
    window.toggleDropdown = function (id) {
        ...
        document.querySelectorAll('.notification-dropdown, .profile-dropdown')
            .forEach(d => { if (d.id !== id) d.classList.remove('is-visible'); });
    ...
    document.addEventListener('click', e => {
        const isTrigger = e.target.closest('#ntToggle') || e.target.closest('.profile-trigger');
        const isInside  = e.target.closest('.notification-dropdown') || e.target.closest('.profile-dropdown');
    ...
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape') { _closeAllActionMenus(); window.closeModal?.(); }
    });
    ...
    // ACTION HANDLER
    const STATUS_MAP = {
        complete: { label: 'Completed',  cls: 'status-completed',  toast: 'success', msg: 'marked as Completed'  },
        process:  { label: 'Processing', cls: 'status-processing', toast: 'info',    msg: 'marked as Processing' },
        pending:  { label: 'Pending',    cls: 'status-pending',    toast: 'warning', msg: 'marked as Pending'    },
    };
    ...
    function _showOrderModal (row, orderId) {
        ...
        const status   = cells[4]?.querySelector('.status-pill')?.textContent.trim() || '—';
        ...
        const html = `
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.85rem;">
                ${[['Order ID', orderId], ['Customer', customer], ['Service', service], ['Amount', amount], ['Status', status]].map(([k,v]) => `
    ...
    // TODO: Sync expiry with backend — server-side cleanup preferred for production
    ...
    window.markAllNotificationsAsRead = function () {
        ...
        _refreshList();
        showToast('Done', 'All notifications marked as read. They will be removed after 8 hours.', 'info');
        // BACKEND INTEGRATION POINT
        // Endpoint: /api/admin/notifications
        // Method: POST
    };
    ...
    window.pushGlobalNotification = function (nt) {
        ...
        if (nt.type === 'order') {
            ...
            orderBufferTimer = setTimeout(() => {
                ...
                if (count > 1) {
                    window.showGlobalToast('Orders Received', `You have ${count} new orders.`, 'success');
                } else {
                    const single = orderBuffer[0];
                    const msg = single.customer ? `${single.customer} — ${single.service}` : single.service;
    ...
    window.updateGlobalUserInfo = function (data) {
        if (data.name)  document.querySelectorAll('[data-user-name]').forEach(el  => el.textContent = data.name);
        if (data.email) document.querySelectorAll('[data-user-email]').forEach(el => el.textContent = data.email);
    ...
    window.showGlobalToast = window.showToast = function (title, message = '', type = 'success') {
    ...
    function _processToastQueue() {
        ...
        const displayTxt = message ? `<strong>${title}</strong> — ${message}` : `<strong>${title}</strong>`;
        toast.innerHTML = `
    ...
    window.toggleModal = function (id, action = 'open', html = null) {
        ...
        if (action === 'open') {
            ...
            setTimeout(() => { overlay.style.display = 'none'; activeModal = null; }, 300);
        }
    };
    ...
    // ============================================================
    document.getElementById('refreshOrdersChart')?.addEventListener('click', function () {
        ...
        ordersPerServiceChart = new Chart(canvas, {
            ...
            options: {
                ...
                scales: {
                    ...
                    y: {
                        grid: { display: false },
                        ticks: { color: '#e8eaf0', font: { size: 12, weight: '600' }, padding: 12 },
    ...
    setTimeout(() => {
        ...
        animateValue('kpi-count-revenue', 0, dashboardData.todayRevenue,  1000, '₱ ');
    }, 200);
