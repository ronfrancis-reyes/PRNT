...
function displayOrders(orders) {
	...
	if (orders.length == 0) {
      ...
      <div>
        ...
        <div id="itemsContainer" style="display:flex; gap:1.5rem; font-weight:700; color:var(--text-dark);">
...
function displayItems(items) {
	...
	items.forEach((item) => {
		container.append(
			`<span><i class="fas fa-print" style="color:var(--primary);"></i> ${item.service_name}: ${item.file_name}</span>`,
		);
	});
}
